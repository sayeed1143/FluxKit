...
import { Video, Upload, CheckCircle, File as FileIcon } from 'lucide-react';
  ...
  const handleConvert = () => {
    ...
    const interval = setInterval(() => {
      setProgress(prev => {
        ...
        return prev + 5; // Slower for "video"
      });
    }, 500);
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-brand-muted max-w-2xl mx-auto">
            Change video formats between MP4, AVI, MOV, and more.
          </p>
        </motion.div>
        ...
        >
          {!file ? (
            <div className="border-2 border-dashed border-brand-border rounded-lg p-12 text-center">
              ...
              <p className="text-brand-muted mb-4">Drag and drop your video file here</p>
              <input type="file" accept="video/*" onChange={handleFileChange} className="hidden" id="video-upload" />
              <label htmlFor="video-upload" className="inline-flex items-center space-x-2 bg-accent text-accent-foreground px-6 py-3 rounded-lg cursor-pointer hover:bg-accent/90" data-cursor-hover>
